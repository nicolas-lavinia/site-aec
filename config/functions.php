<?php
	...
	// ********************************************************************
	// função para verificar se a string é formada por caractéres numéricos
	// ********************************************************************
	function retorna_num($args)
	{
		$result = 0;
    ...
    function login_config( $login, $password, $ignora_data )
    {
        require('../comum/db_connect.php');
        // utilizar declarações preparadas significa que a injeção de código SQL não será possível. 
        if ($stmt = $mysqli->prepare("SELECT CodUsu, Senha, CodPerfil, ExpiraEm, Ativo FROM cadusu WHERE Login = ? LIMIT 1"))
        { 
            $stmt->bind_param('s', $login ); // Vincula "$email" ao parâmetro.
            $stmt->execute(); // Executa a query preparada.
            $stmt->store_result();
            $stmt->bind_result( $user_id, $db_password, $CodPerfil, $expira, $status_user ); // obtém variáveis do resultado.
            $stmt->fetch();
            if( $stmt->num_rows == 1 )
            {   // se o usuário existe
                ...
                { // Checa se a senha na base de dados confere com a senha que o usuário digitou. 
                  // Senha está correta!
                	...
                	{
                		if ($stmt = $mysqli->prepare("SELECT opc_01 FROM perfil WHERE CodPerfil = ? LIMIT 1"))
	                	{
	                		$stmt->bind_param('i', $CodPerfil );
	                		$stmt->execute();
	                		$stmt->store_result();
	                		$stmt->bind_result( $opc_01 ); // obtém variáveis do resultado.
	                		if( $stmt->fetch() )
	                		{
								if( $opc_01 == 1 )
								{
