<?php
    ...
    function login_oem_user( $login, $password )
    {
    	require('dbconnlic.php');
    	// utilizar declarações preparadas significa que a injeção de código SQL não será possível.
    	if ($stmt = $mysqli->prepare("SELECT id, senha, status FROM users WHERE login = ? LIMIT 1"))
    	{
    		$stmt->bind_param('s', $login ); // Vincula "$email" ao parâmetro.
    		$stmt->execute(); // Executa a query preparada.
    		$stmt->store_result();
    		$stmt->bind_result( $user_id, $db_password, $status_user ); // obtém variáveis do resultado.
    		$stmt->fetch();
    		if( $stmt->num_rows == 1 )
    		{   // se o usuário existe
    			$_SESSION['oem_user_id'] = $user_id;
    			...
    			{ // Checa se a senha na base de dados confere com a senha que o usuário digitou.
    				...
    				$query = "UPDATE users SET ult_login='$agora' WHERE id = '$user_id'";
    				$mysqli->query($query);
    ...
    function login_check( )
    {
        ...
        if(isset($_SESSION['oem_user_id'], $_SESSION['login_string']))
        {
            $user_id = $_SESSION['oem_user_id'];
            ...
            require('dbconnlic.php');
            if ($stmt = $mysqli->prepare("SELECT senha, status FROM users WHERE id = ? LIMIT 1"))
            { 
                ...
                {   // Caso o usuário exista
                    $stmt->bind_result($password, $status_user); // pega variáveis a partir do resultado
                    $stmt->fetch();
                    $login_check = hash('sha512', $password.$ip_address.$user_browser);
                    if( ( $login_check == $login_string ) && ( $status_user == 1 ) )
                    {
                    	// Logado!!!
	                    $stmt->close();     // fecha a declaração
	                    $mysqli->close();   // fecha a conexão
